...
export function PeerBars({ dados }) {
  const maxX = Math.max(...dados.barras.map((p) => p.x))
  return (
...
export function PeerPercentile({ dados }) {
  return (
    <div className="body" style={{ display: 'flex', flexDirection: 'column' }}>
      <svg width="100%" height="100%" viewBox="0 0 200 110" preserveAspectRatio="none" style={{ flex: 1 }}>
        ...
        <text x="150" y="11" fontSize="9" fill={C.orange} fontWeight="700">P{dados.percentil}</text>
      </svg>
      <div className="cap">{dados.descricao}</div>
    </div>
  )
}

export function PeerRadar({ dados }) {
  return (
    <div className="body">
      <ResponsiveContainer width="100%" height="100%">
        <RadarChart data={dados.eixos} outerRadius="70%">
